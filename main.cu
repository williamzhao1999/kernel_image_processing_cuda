...
int main()
{
    ...
    //for(int k = 0; k < 50; k++) {
        ...
        for (int i = 0; i < images.size(); i++) {
            ...
            cudaMemcpy2D(input_data_gpu_red, pitch_r, input_image_pixel_red, width, width, height,
                         cudaMemcpyHostToDevice);
            cudaMemcpy2D(input_data_gpu_green, pitch_g, input_image_pixel_green, width, width, height,
                         cudaMemcpyHostToDevice);
            cudaMemcpy2D(input_data_gpu_blue, pitch_b, input_image_pixel_blue, width, width, height,
                         cudaMemcpyHostToDevice);
            ...
            for (int j = 0; j < 1; j++) {
                ...
                convolution<<<gridSize, blockSize>>>(output_data_gpu_green, input_data_gpu_green, pitch_g, kernel,
                                                     factorKernel, width, height);
                convolution<<<gridSize, blockSize>>>(output_data_gpu_blue, input_data_gpu_blue, pitch_b, kernel,
                                                     factorKernel, width, height);
